"""When does a real low block leave its shape to press?

The scripted press in `defenders.py` picks whoever is closest to the ball and
sends them at it at full speed, from anywhere on the pitch. Against a policy that
recycles the ball every tick that produces one defender ping-ponging across the
block and a hole where they used to stand. This study measures what a real
defending team does instead: how far up the pitch the ball has to be before
anyone steps out, how many step out, how far they travel, and how long a
commitment lasts.

TWO SOURCES, AND THEY ARE NOT INTERCHANGEABLE
---------------------------------------------
1. StatsBomb low-block freeze frames (`low_block_frames.csv`,
   `low_block_defenders.csv`) -- 2032 frames already filtered to settled low
   blocks by calibration_graphing.py, see README section 1. This is the source of
   truth for every GEOMETRIC quantity here: engagement distance, how much of the
   block commits, what it costs the shape. It is the right population by
   construction.

2. Metrica Sample_Game_1 tracking -- one full match at 25fps. A freeze frame is a
   single instant, so it physically cannot measure a duration or a path length,
   and those are exactly what the press latch needs. Metrica is used for those
   two numbers ONLY, and only on frames whose block depth falls inside the range
   the StatsBomb low blocks actually occupy (BLOCK_DEPTH_MIN_SIM below, set to
   the StatsBomb q05). A full match is mostly not a low block -- unfiltered it
   would be measuring mid-block and counter-pressing, which is a different
   behaviour with different numbers.

The two overlap on d1 and n_near, and fig_engagement plots both, so you can see
whether the Metrica subset really is describing the same phase before trusting
the durations taken from it. Treat everything from source 2 as
order-of-magnitude: one match, not eight.

Coordinate frame: everything below is in the SIM frame -- the defending team
defends x=105, attacks x=0, pitch 105x68. That is the frame `defenders.py` works
in, so the parameters this writes out drop straight in without mirroring. The
StatsBomb CSVs are stored in the calibration frame (x=0 at the defended goal);
`x_sim = 105 - x_cal` converts, and distances are invariant.

Outputs, all into this directory:
    press_engagement.png       how close the ball gets before anyone engages
    press_commitment.png       how many defenders leave the block at once
    press_shape.png            which line presses, and what the block pays
    press_duration.png         how long one commitment lasts, and how far it runs
    press_frames.csv           the per-frame StatsBomb table
    press_spells.csv           the per-spell Metrica table
    press_policy_params.csv    the derived parameters

Usage:
    python defenders/calibration/press_calibration.py
"""
