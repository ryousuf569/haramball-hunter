"""Is the task reachable at all on the calibrated duel gate?

This is a SIM SWEEP, not a data fit -- README's second category. duel_calibration.py
fits DUEL_A and LAM_MAX to Metrica; this measures what those numbers do to the
environment, which is a separate question and is not answered by the fit being
good.

In the 500k run both constrained arms scored 0/150, and 103 of those 150 episodes
ended in a lost ground duel. A zero success rate cannot tell an unreachable
terminal from a policy that has not found it yet, so before spending another 500k
steps it is worth measuring the floor directly: run the SAME episodes on the old
gate and the new one and see whether the outcome mix moves.

Two policies are run at every gate.
  * random   -- the base rate. Whatever a uniformly random policy scores is what
                the environment concedes before any learning happens, and it is
                the control diag_policy.py insists on for the same reason.
  * the 500k constrained checkpoint -- the arm that scored 0/150. If its duel
                losses fall away on the calibrated gate, the gate was the binding
                constraint; if they do not, something else is.

Four gates are run, not two: they are the successive stages of README section 7 --
RoboCup as transferred, then the fitted rate, then the fitted reach, then the
fitted axes -- so the sweep separates what the rate did from what the shape did.

Every arm sees the same seeds and the same starting holders, so a difference
between two rows is the duel gate and nothing else. Episodes are run through
scripts/diag_policy.py's own runner rather than a second copy of it.

MATCHED RANDOMNESS. run_episode draws actions with dist.sample(), which uses
torch's GLOBAL rng -- process state, not episode state. Under pool.map that made
an episode's actions depend on how many episodes its worker ran first, and the
arms were not matched at all. An earlier version of this sweep reported duel
losses falling 29 -> 15 across the gates on that basis; it was the harness, not
the physics. worker() now seeds torch per episode. Discard any duel-gate
comparison produced before that fix.

Outputs, into this directory:
    duel_gate_recheck_runs.csv     one row per episode
    duel_gate_recheck_summary.csv  one row per (gate, policy) cell

Usage:
    python defenders/calibration/duel_gate_recheck.py [--n 40] [--workers 8]
"""
