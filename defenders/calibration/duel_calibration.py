"""How fast does a real defender actually take the ball off a carrier?

`turnover.py`'s `LAM_MAX` is the last physics constant in the project that was
never fitted to football. It came from a target rather than a measurement -- "a
perfectly positioned defender should win the ball with probability 0.65 over half
a second", so `lambda = -ln(1 - 0.65) / 0.5 = 2.1 /s`. README section 7 records
that target and also records that nothing in the data ever justified the 0.65 or
the 0.5s. `DUEL_A = 1.20` has the same problem one level down: it is RoboCup's
`tackle_dist` rescaled by eye, and because `DUEL_EXP = 6` makes the super-ellipse
a near-box, it acts as a hard on/off switch. Together they say a carrier is
untouchable at 1.21m and has an 81% chance of losing the ball within a second at
1.00m, with nothing in between.

Metrica Sample_Game_1 can measure both, because its event file labels ground
duels explicitly (`CHALLENGE` with a `GROUND-*` / `TACKLE-*` subtype, plus
`THEFT`) while its tracking gives the exposure those events happened during. That
turns the question into a survival problem with a closed-form answer:

    lambda_hat = (duel losses inside the gate) / (time spent inside the gate)

the standard occupancy estimator for a constant hazard. Both halves are measured
here from the same construction, which is what makes the ratio a hazard rate
rather than two unrelated counts.

WHAT IS FITTED, AND WHAT IS NOT
-------------------------------
Fitted: all three semi-axes of the duel gate (`DUEL_A`, `DUEL_B`, `DUEL_A_BACK`)
and the hazard rate `LAM_MAX`. `DUEL_EXP` is fitted too, but the answer is that 53
losses cannot identify it -- see fit_exponent -- so it keeps RoboCup's 6, now on
evidence rather than on inheritance.

The axis ratios USED to be transferred from RoboCup rather than fitted, and that
turned out to be the model's biggest error, bigger than the rate. RoboCup's
tackle box is strongly forward-biased (A/B = 2.4 as implemented), but real
dispossessions are not: the p90 of |dy| across the defender's heading is 2.08m
against 2.05m along it, so a duel is as wide as it is deep.

That blindness, not the rate, was the model's main error. The original RoboCup
gate saw 14 of 53 real ground-duel losses; rescaling it to the right reach while
keeping RoboCup's ratios only got that to 28 of 53. Since `ground_duel` is the
model's ONLY channel for taking the ball off a carrier, a gate blind to half of
them cannot deliver the right rate per contest at any `LAM_MAX` -- which is why
the sim's 1v1 stayed too attacker-friendly even after the rate was fitted.

Each axis is anchored at the p90 of the coordinate it governs, then all three are
scaled by a common factor until the gate jointly encloses 90% of real
dispossessions -- `DUEL_EXP = 6` cuts the corners, so three independent p90 axes
enclose well under 90% together. A reach is a cap, so anchoring high is the same
argument, and the same quantile family, as `ANCHOR_Q` in
dribble_speed_calibration.py.

Two radial readings cross-check the result rather than setting it: the gap at
which the model-free hazard profile collapses, and the p90 defender-to-ball gap
at a real dispossession.

Outputs, all into this directory:
    duel_hazard_profile.csv    model-free hazard vs gap, the core evidence
    duel_loss_events.csv       every matched ground duel, with its geometry
    duel_gate_scan.csv         lambda re-fitted at each candidate gate scale
    duel_exponent_profile.csv  DUEL_EXP profile likelihood, scale free
    duel_engagement_profile.csv  model-free hazard by closing speed
    duel_engagement_fit.csv    (ADV_FLOOR, ADV_EXP) likelihood grid
    duel_survival_params.csv   the derived constants
    duel_calibration_fit.png   the four panels behind them

Usage:
    python defenders/calibration/duel_calibration.py
"""
