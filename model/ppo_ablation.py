...
class Config:
    ...
    ablation: str = "control" # recorded in config.json so runs are self-describing
...
def team_slow_indicator(obs):
    """Ablation: the same threshold read off the TEAM MEAN speed instead of each
    attacker's own. One bit per tick, broadcast over the squad. Baseline play
    already sits near 1.0 here, so the constraint saturates and the multiplier
    has almost no gradient to act on."""
    speed = torch.linalg.norm(obs[..., OWN_VEL:OWN_VEL + 2], dim=-1) * V_MAX
    fires = (speed.mean(dim=-1) < SPEED_LIMIT).float()
    return fires[:, None].expand(-1, obs.shape[-2])
...
def collect_rollout(agent, venv, buf, state, tracker, zone_centre, zone_radius,
    ...
    for t in range(buf.T):
        ...
        buf.cost[t, :, :, 0] = slow_fn(state.next_obs)
...
class SoftplusMultipliers(Multipliers):
    """Ablation: lam_k = softplus(z_k), each independent and unbounded, with the
    reward weight pinned at 1 instead of falling out of the simplex. Removes the
    only thing bounding sum(lam), so this is the test of whether the simplex is
    doing work or was never going to bind here."""

    def forward(self):
        return torch.ones((), device=self.z.device), F.softplus(self.z)


MULTIPLIERS = {"softplus": SoftplusMultipliers}
SLOW_INDICATORS = {"team_slow": team_slow_indicator}

ABLATIONS = {
    "control": "unmodified, matches ppo_constrained.py",
    "no_bootstrap": "success floor disabled via --succ-d 0.0, so lam_succ decays "
                    "and lam0_tilde = max(lam0, lam_succ) collapses to lam0",
    "softplus": "lam_k = softplus(z_k), unbounded, reward weight pinned at 1",
    "team_slow": "slow read off the team mean speed, not each attacker's own",
}
...
def banner(cfg, n_updates, obs_d):
    ...
    print("ABLATION %s: %s" % (cfg.ablation, ABLATIONS[cfg.ablation]))
...
def train(cfg):
    ...
    # the only two ablations that need code; no_bootstrap is reached by setting
    # succ_d to 0.0, which leaves the floor satisfied and lam_succ at ~0
    mult_cls = MULTIPLIERS.get(cfg.ablation, Multipliers)
    slow_fn = SLOW_INDICATORS.get(cfg.ablation, slow_indicator)
    mult = mult_cls(constraints_from(cfg), cfg.z_init)
    ...
    for update in range(1, n_updates + 1):
        ...
        state = collect_rollout(agent, venv, buf, state, tracker,
                                zone_centre, cfg.zone_radius, slow_fn)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--ablation", choices=sorted(ABLATIONS), default="control",
                    help="; ".join("%s: %s" % kv for kv in ABLATIONS.items()))
    args = ap.parse_args()
    if args.ablation == "no_bootstrap" and args.succ_d != 0.0:
        raise SystemExit("no_bootstrap needs --succ-d 0.0 to disable the floor, "
                         "got %.2f" % args.succ_d)
