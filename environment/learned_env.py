...
The three encodings below -- observation, ball mask, action decoding -- are
clones of LowBlockEnv.obs / action_mask / decode_action and MUST stay
byte-identical to them. A policy fed a differently-ordered observation than it
was trained on does not error, it just plays badly, so treat any edit to
lowblock_env's obs layout as an edit to this file too.
...
def load_actor(path, n_att=10, n_def=11, device="cpu"):
    ...
    actor = Actor(obs_dim(n_att + n_def), N_DIRECTIONS, N_SPEEDS,
...
def obs(players, ball, n_att, tick, max_ticks):
    """(n_att, obs_dim) float32 -- clone of LowBlockEnv.obs."""
...
def learned_step(players, ball, attacker_ids, defender_state, tick, actor,
                 ppcf_grid=None, max_ticks=300, deterministic=True,
    ...
    o = obs(players, ball, n_att, tick, max_ticks)
...
def run_episode(actor, n_att=10, n_def=11, seed=54, max_ticks=300,
    ...
    for tick in range(max_ticks):
        players, ball, _pc, outcome = learned_step(
